...
from CpuControlGreed import CpuControlGreed
#====================================================================
...
class ChessGame:
    ...
    def __init__(self):
        ...
        self.cpu_control_greed = CpuControlGreed(self.move_calculator, self.check_game_end)
    ...
    def chess_run(self):
        ...
        while self.process_flag:
            # 白ターンの時はクリックイベントを待つ（黒ターンはAI）
            if (self.turn_manager.turn):
                click_pos = self.wait_events()
                self.change_state(click_pos)
            else:
                cpu_ope = self.cpu_control_greed.create_tmp_board(self.board_manager.curr_board, self.turn_manager.turn, self.logger.moved_pieces_log, self.logger.enpassant_log, self.logger.game_log)
                # 返ってきたcpuの情報がダミー情報の場合がある（チェックメイト、ステルスメイトのとき）
                self.select_destination(cpu_ope[2][0], cpu_ope[2][1], cpu_ope)
    ...
    def select_destination(self, destY, destX, cpu_ope=None):
        """
        移動先の選択を待ち、移動実行を行う
        引数：
            destY, クリックされた移動先セルのY座標
            destX, クリックされた移動先セルのX座標
            cpu_ope = cpuの動作['駒', (出発Y, 出発X), (行先Y, 行先X)]
        戻り値：
            なし
        """
        if cpu_ope:
            # CPUの場合
            piece = cpu_ope[0]
            fromY, fromX = cpu_ope[1]
        else:
            # プレイヤーの場合は、選択されたセルが移動可能セルのリストにない場合は駒選択に戻る
        ...
        if (piece != "xX"): # ダミー情報でないとき
            if piece[1] == "P":
                ...
                self.move_executor.move_common(self.board_manager.curr_board, (fromY,fromX), (destY,destX), piece, self.move_calculator.castling_flag)
