...
class CheckGameEnd:
    ...
    def is_checkmate(self, curr_board, turn, prev_log):
        """
        チェックメイトの判定を行う
        チェックメイトならTrueを返す
        引数；
        ...
        ret, _ = self.check_legal_moves(curr_board, turn, prev_log)
        if ret:
            return False
    ...
    def check_legal_moves(self, curr_board, turn, prev_log):
        """
        合法手をがあるかどうかを確認する
        合法手が存在すればTrueを返す
        引数；
        ...
        for r_idx, row in enumerate(curr_board):
            for c_idx, cell in enumerate(row):
                ...
                if (cell != 0) and (cell[0] == allies):
                    if (cell[1] == "P"):
                        mobiles = move_calculator.calc_pawn_moves(curr_board, r_idx, c_idx, prev_log, turn)
                    else:
                        mobiles = move_calculator.calc_common_moves(curr_board, r_idx, c_idx, cell, turn) 
                    # 移動先が得られなければスキップ
                    if len(mobiles) == 0:
                        continue
    ...
    def is_stalemate(self, curr_board, turn, prev_log):
        """
        ステルスメイトのチェックを行う
        ステルスメイトであればTrueを返す
        引数；
