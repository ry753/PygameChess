...
class ChessGame:
    ...
    def select_piece(self, cellY, cellX):
        ...
        if cell[1] == "P":
            ...
            mobiles = self.move_calculator.calc_common_moves(self.board_manager.curr_board, cellY, cellX, cell, self.turn_manager.turn)
    ...
    def select_destination(self, destY, destX):
        ...
        if self.check_game_end.is_checkmate(self.board_manager.curr_board, self.turn_manager.turn, self.logger.enpassant_log):
            self.game_state = "END_GAME"
            self.in_checkmate = True
        # チェックメイトでない場合はステルスメイトの確認も行う
        else:
            if self.check_game_end.is_stalemate(self.board_manager.curr_board, self.turn_manager.turn, self.logger.enpassant_log):
                self.game_state = "END_GAME"
