...
class CheckGameEnd:
    ...
    def is_checkmate(self, curr_board, turn, prev_log, moved_pieces):
        """
        チェックメイトの判定を行う
        チェックメイトならTrueを返す
        引数；
        ...
        ret, _ = self.check_legal_moves(curr_board, turn, prev_log, moved_pieces)
        if ret:
            return False
    ...
    def check_legal_moves(self, curr_board, turn, prev_log, moved_pieces):
        """
        合法手をがあるかどうかを確認する
        合法手が存在すればTrueを返す
        引数；
    ...
    def is_stalemate(self, curr_board, turn, prev_log, moved_pieces):
        """
        ステルスメイトのチェックを行う
        ステルスメイトであればTrueを返す
        引数；
